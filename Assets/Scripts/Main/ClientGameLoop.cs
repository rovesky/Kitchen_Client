...
namespace FootStone.ECS
{
    ...
    //    public GameTick PredictedTime
    //    {
    //        get { return m_PredictedTime; }
    //    }

    //    public GameTick RenderTime
    //    {
    ...
    //        m_GameWorld.WorldTick = m_RenderTime;
    ...
    //        // Prediction
    //        m_GameWorld.WorldTick = m_PredictedTime;
    ...
    //            m_GameWorld.WorldTick.SetTick(m_NetworkClient.serverTime, m_PredictedTime.TickInterval);
    //            PredictionRollback();
    ...
    //            {
    //                m_GameWorld.WorldTick.SetTick(Tick, m_PredictedTime.TickInterval);
    //                m_PlayerModule.RetrieveCommand(m_GameWorld.WorldTick.Tick);
    //                PredictionUpdate();
    //#if UNITY_EDITOR                 
    ...
    //            if (m_GameWorld.WorldTick.tickDuration > 0.008f) 
    //            {
    //                PredictionUpdate();
    //            }
    ////#if UNITY_EDITOR                 
    ...
    //        int preferredTick = m_NetworkClient.serverTime + (int)(((m_NetworkClient.timeSinceSnapshot + m_NetworkStatistics.rtt.average) / 1000.0f) * m_GameWorld.WorldTick.TickRate) + preferredBufferedCommandCount;
    ...
    //            m_PredictedTime.SetTick(preferredTick, 0);
    ...
    //        {
    //            m_RenderTime.SetTick(m_NetworkClient.serverTime, 0);
    //        }

    //        // hard catchup
    //        if (m_RenderTime.Tick < m_NetworkClient.serverTime - 10)
    //        {
    //            m_RenderTime.SetTick(m_NetworkClient.serverTime - 8, 0);
    //        }
    ...
    //    GameTick m_PredictedTime = new GameTick(60);
    //    GameTick m_RenderTime = new GameTick(60);
    ...
    public class ClientGameLoop : Game.IGameLoop
    {
        ...
        //            m_NetworkStatistics.Update(m_clientWorld.frameTimeScale, GameTick.GetDuration(m_clientWorld.RenderTime, m_clientWorld.PredictedTime));
